...
class LabelEmbedder(nn.Module):
    ...
    """
    def __init__(self, num_classes, hidden_size, dropout_prob, device=None):
        super().__init__()
    ...
    def forward(self, labels, train, force_drop_ids=None):
        ...
        embeddings = self.embedding_table(labels.clone().detach().to(self.device))
        return embeddings
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y):
        # input("stop here modelpy: ")
        x = x.to(self.device)
        x = self.x_embedder(x) + self.pos_embed
        # input("stop here again modelpy: ")
        t = self.t_embedder(t)
        y = self.y_embedder(y, self.training)  # dropout layer is only applied during training because it is wrapped with the self.training flag
        c = t + y
        for block in self.blocks:
            x = block(x, c)
